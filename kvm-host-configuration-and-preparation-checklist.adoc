...
+
Installare il sistema operativo RHEL utilizzando l'immagine ISO. Vedere link:reference_plan_ots_hardware.html#software-compatibility["informazioni sulla compatibilità del software per gli hypervisor"] per un elenco delle versioni RHEL supportate. Durante l'installazione, configurare il sistema come segue:
...
NOTE: Alcuni dei pacchetti richiesti potrebbero essere già installati se si è scelto l'host virtualizzato per la selezione del software durante l'installazione del server Linux. Potrebbe essere necessario installare il pacchetto openvswitch dal codice sorgente, come descritto in link:https://docs.openvswitch.org/en/latest/intro/install/general/["Apri la documentazione di vSwitch"^] .

+
Per ulteriori informazioni sui pacchetti necessari e altri requisiti di configurazione, vedere link:https://imt.netapp.com/matrix/#welcome["Strumento di matrice di interoperabilità"^] .
