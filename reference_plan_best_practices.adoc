...
NOTE: Durante l'installazione su ESX 6.5 U2 e versioni successive, ONTAP Select utilizza il driver vNVME indipendentemente dal fatto che il disco di sistema risieda su un SSD o su un disco NVME. Questo imposta il livello hardware della VM a 13, compatibile con ESX 6.5 e versioni successive.
...
Per saperne di più su come configurare un Open vSwitch su un host hypervisor, vedere link:https://www.netapp.com/media/13134-tr4613.pdf["ONTAP Select sull'architettura del prodotto KVM e sulle migliori pratiche"^] rapporto tecnico.
