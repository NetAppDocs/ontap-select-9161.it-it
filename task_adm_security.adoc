...
|===
. Dopo aver aggiunto il nuovo server di gestione, è possibile fare clic facoltativamente su image:icon_kebab.gif["Opzioni"] e seleziona una delle seguenti opzioni:
+
...
Se si sceglie di optare per l'autenticazione FIDO2 YubiKey, configurare il PIN FIDO2 YubiKey utilizzando YubiKey Manager e generare la chiave FIDO2 con una PuTTY-CAC (Common Access Card) per Windows o ssh-keygen per macOS. I passaggi per farlo sono riportati nel rapporto tecnico. link:https://docs.netapp.com/us-en/ontap-technical-reports/security.html#multifactor-authentication["TR-4647: Autenticazione multifattoriale in ONTAP"^] .
